...
- **주입 방식**: 여러 줄 프롬프트는 브래킷 페이스트(`\x1b[200~…\x1b[201~`)로 한 번에 넣고 짧은 지연 후 Enter(`\r`)로 제출(조기 제출 방지). 슬래시 명령(`/compact`·`/clear`)도 동일.
...
상단 툴바 없음. 단일 윈도우 **상/하 수직 분할**:

```
┌──────────────────────────────────────────────┐
│ 메인 Claude Code        [활동표시]      [⚙]   │  ← 타이틀바
│                                               │
│        Claude Code PTY (xterm)                │  (상단 ~70%)
│                                               │
├──────────────────────────────────────────────┤  ← 드래그 리사이즈
│ Hook 콘솔                              [🗑]    │
│  [HH:MM:SS] [HOOK] panel=main-claude ...      │  (하단 ~30%)
│  [HH:MM:SS] [SYSTEM] 설정 저장 ...            │
└──────────────────────────────────────────────┘
```

- 분할 비율은 `react-resizable-panels` + localStorage(`app-rows`)로 저장/복원.
- 백그라운드 컴포넌트: `GateModal`(PreToolUse 게이트), `HookBridge`(이벤트 리스너).

### 4.2 영역별 역할

- **상단 — 메인 Claude Code 패널**: PTY 렌더링. 타이틀바에 활동 인디케이터(thinking/idle)와 ⚙ 설정 버튼.
- **하단 — Hook 콘솔**: `consoleEvents`를 시간순으로 표시(`[HOOK]`/`[SYSTEM]`/`[USER]`). 🗑로 비우기.
