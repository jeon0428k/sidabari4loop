...
## 2. 작업 정의 — `docs/BUILD_ORDER.md` (운영 프롬프트 참조 + '진행' 화면이 읽음)

'다음 한 단계'를 결정할 수 있는 단계/순서 정의이자, **전체 진행 목록의 단일 원본**이다.

**형식(체크박스 — 진행 표시의 기준):**
- 큰 묶음은 `## Phase N — 제목` 헤딩, 세부 단계는 `- [ ] STEP-ID 설명` 체크박스로 적는다.
- 자율 루프가 각 step의 검증 게이트를 통과하면 그 줄을 `- [x]`로 바꾼다.

> 근거: `src/lib/buildOrder.ts` `parseBuildOrder`(체크박스만 결정론적으로 검출 — `- [x]` 완료),
> `src/hooks/useBuildProgress.ts`(매 Stop마다 다시 읽음), '진행' 탭(`ProgressPanel`).

이제 **Sidabari4Loop이 이 파일을 직접 읽으므로**(진행 화면) **경로는 고정** `docs/BUILD_ORDER.md`이며,
운영 프롬프트가 가리키는 이름과 일치해야 한다. `TASK_COMPLETE`/`OPEN:` 마커가 정규식으로 검출되는 것과
같은 철학으로, 진행 여부는 **자유 산문이 아니라 체크박스로만** 판정한다.
...
**권장(필수 아님):** 도메인 로직 step은 끝내기 전, 이번 step의 변경(diff)을 **새 컨텍스트의 리뷰
서브에이전트**로 검토한다(예: Task의 code-reviewer, 또는 `/code-review`). 자기 컨텍스트 자기검토의
편향을 줄이는 "새 눈"이 목적이다. **정확성·보안·회귀 관점의 실제 결함만** 보고(스타일 사소한
지적으로 턴을 막지 말 것), 고치거나 못 풀면 `OPEN:`으로 남긴다. 이는 운영 관행 권장이며 **§1·§4의
기계적 계약(경로·마커·훅)이 아니다** — Sidabari4Loop는 리뷰 수행 여부를 강제·검출하지 않는다.
...
| **UserPromptSubmit** | 운영 프롬프트가 실제로 제출됨 → 제출 워치독 해제(미제출 시 Enter 재전송) | 권장(미설치면 워치독이 보수적으로 동작) |
| **Notification(idle_prompt)** | Claude 멈춤(idle) → 운영 프롬프트 재주입 넛지 | 스톨 자동 복구 |

> 근거: `SupervisorController.tsx` listener — `kind==="stop"`, `kind==="session-start" && payload.source===expected`, `kind==="user-prompt"`, `kind==="notification" && notification_type==="idle_prompt"`.
